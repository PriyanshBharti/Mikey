...
def get_id(update: Update, context: CallbackContext):
    ...
    user_id = cextract_user(msg, args)

    if str(user_id).startswith('-100'):
       CHAT_INFO = bot.get_chat(user_id)
       msg.reply_text(
           f"{html.escape(CHAT_INFO.title)}'s id is <code>{CHAT_INFO.id}</code>.",
...
def info(update: Update, context: CallbackContext):
    ...
    message = update.effective_message
    chat = update.effective_chat
    user_id = cextract_user(update.effective_message, args)

    # Channel Info
    if str(user_id).startswith('-100'):
       CHANNEL_INFO = bot.get_chat(user_id)
       text = f'''
<b>Channel Name:</b> <code>{CHANNEL_INFO.title}</code>
<b>Channel ID:</b> <code>{CHANNEL_INFO.id}</code>
<b>Channel USERNAME:</b> @{CHANNEL_INFO.username}
<b>Channel Link:</b> <a href="t.me/c/{str(CHANNEL_INFO.id).replace('-100', '')}/1">Link</a>
...
{CHANNEL_INFO.description}
       ...
       try:
          bot.send_photo(
            chat.id,
            photo=CHANNEL_INFO.photo.small_file_unique_id,
            caption=(text),
            parse_mode=ParseMode.HTML,            
        )
          return
    ...
    if user_id not in [bot.id, 777000, 1087968824]:
        ...
        text += f"\n\n<b>Health:</b> <code>{userhp['earnedhp']}/{userhp['totalhp']}</code>\n[<i>{make_bar(int(userhp['percentage']))} </i>{userhp['percentage']}%]. [<a href='https://t.me/Shinobu_Update_Channel/8'>?</a>]"
    ...
    if user.id == OWNER_ID:
        ...
        text += "\n\n It's a 'Hashira' ranked slayer just like me."
        disaster_level_present = True
    elif user.id in DEMONS:
        text += "\n\nThe rank of this slayer is 'Kinoe'."
        disaster_level_present = True
...
def stats(update, context):
    ...
    try:
        update.effective_message.reply_text(
            ...
            + f"\n\n[✦ Support](https://t.me/{SUPPORT_CHAT}) | [✦ Updates](https://t.me/KanekiUpdates)\n\n"
            + "╘══「 by [Hiro](https://t.me/Darling_Hiro) 」\n",
            parse_mode=ParseMode.MARKDOWN,
            disable_web_page_preview=True,
        )
