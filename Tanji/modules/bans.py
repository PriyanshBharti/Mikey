...
from Tanji.modules.helper_funcs.extraction import extract_user_and_text
from Tanji.modules.helper_funcs.string_handling import extract_time
from Tanji.modules.log_channel import gloggable, loggable
...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            message.reply_text("Trying to put me against a Sun Breather?")
        elif user_id in DEV_USERS:
            message.reply_text("I can't act against Flame Breather.")
        elif user_id in DRAGONS:
            message.reply_text(
                "Fighting this Thunder breathers here will put user lives at risk."
            )
        elif user_id in DEMONS:
            message.reply_text(
                "Bring an order from DemonslayerCorps to fight a Wind breather"
            )
        elif user_id in TIGERS:
            message.reply_text(
                "Bring an order from Corps members fight a Water Breather."
            )
        elif user_id in WOLVES:
            message.reply_text("Water breather access make them ban immune!")
        else:
