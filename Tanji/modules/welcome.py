...
import Tanji.modules.sql.welcome_sql as sql
from Tanji import (
    DEV_USERS,
    OWNER_ID,
    DRAGONS,
...
from Tanji.modules.helper_funcs.misc import build_keyboard, revert_buttons
from Tanji.modules.helper_funcs.msg_types import get_welcome_type
from Tanji.modules.helper_funcs.handlers import MessageHandlerChecker
from Tanji.modules.helper_funcs.string_handling import (
        ...
        if should_welc:
            ...
            if new_mem.id == OWNER_ID:
                update.effective_message.reply_text(
                    f"Welcome to {html.escape(chat.title)}.", reply_to_message_id=reply
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
                    f"#USER_JOINED\n"
                    f"A sun breather just arrived"
                )
                continue
            ...
            if new_mem.id in DRAGONS:
                update.effective_message.reply_text(
                    "Huh! A Thunder Breather! Stay Alert maybe there are demons!",
                    reply_to_message_id=reply,
                )
                continue
            ...
            if new_mem.id in WOLVES:
                update.effective_message.reply_text(
                    "Konnichiwa! Beast Breather.", reply_to_message_id=reply
                )
                continue
    ...
    if should_goodbye:
        ...
        if left_mem:
            ...
            if left_mem.id == OWNER_ID:
                update.effective_message.reply_text(
                    "Sayonara! Yoriichi san :(", reply_to_message_id=reply
                )
                return
            ...
            if left_mem.id in DEV_USERS:
                update.effective_message.reply_text(
                    "See you later at the Demon Slayer corps",
                    reply_to_message_id=reply,
                )
                return
